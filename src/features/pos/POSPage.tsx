    ...
    <div className="min-h-screen bg-[var(--unit-surface)]">
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-6">
          <Link
            href="/appointments"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-unit border border-[var(--unit-border)]/50 text-[var(--unit-accent)] font-medium bg-[var(--unit-surface-elevated)] hover:bg-[var(--unit-accent)] hover:text-white transition-colors"
          >
        ...
        {/* Quick Access Bar */}
        <div className="rounded-unit-lg border border-[var(--unit-border)]/30 bg-[var(--unit-surface-elevated)] p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-unit bg-[var(--unit-accent)]">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                {popularServices.map((service: any) => (
                  <button
                    ...
                    className={`relative flex flex-col items-center gap-2 rounded-unit border p-4 text-xs font-medium transition-all active:scale-[0.98] ${service.isPopular
                      ? 'border-[var(--unit-accent)]/50 bg-[var(--unit-accent)]/10'
                      : 'border-[var(--unit-border)]/50 bg-[var(--unit-surface)] hover:border-[var(--unit-accent)]/50'
                      }`}
                  >
                    {service.isPopular && (
                      <div className="absolute -top-1 -right-1 z-10">
                        <Star className="h-3 w-3 text-[var(--unit-warning)] fill-[var(--unit-warning)]" />
                      </div>
                    )}
                    <span className="text-xl">{service.icon || '✂️'}</span>
                    <span className="font-medium text-[var(--unit-text)] text-center leading-tight line-clamp-2">
                      {service.name}
                    </span>
                    <span className="text-[var(--unit-accent)] font-bold text-xs">S/ {getServicePrice(service)}</span>
                    {service.badgeText && (
                      <span className="absolute top-1 left-1 text-[8px] font-bold text-white bg-red-500 px-1.5 rounded">
                        {service.badgeText}
                      </span>
                    )}
                  </button>
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center gap-3">
          {selectedCustomer ? (
            ...
            <div className="relative" ref={clientSearchRef}>
              ...
              {showClientSearch && (
                <div className="absolute left-0 top-full z-20 mt-2 w-80 rounded-[var(--unit-border-radius)] border border-[var(--unit-border)] bg-[var(--unit-surface)] p-3 shadow-[var(--unit-shadow-unit)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 lg:col-span-2 space-y-4">
            ...
            {/* Item Search */}
            <section className="rounded-unit-lg border border-[var(--unit-border)]/30 bg-[var(--unit-surface-elevated)] p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div className="relative z-20" ref={itemSearchRef}>
                  ...
                  <input
                    ...
                    className="w-full rounded-unit border border-[var(--unit-border)]/50 bg-[var(--unit-surface)] pl-12 pr-4 py-3 text-[var(--unit-text)] focus:outline-none focus:ring-2 focus:ring-[var(--unit-accent)]/50 focus:border-[var(--unit-accent)] transition-all placeholder:text-[var(--unit-text-muted)]/50"
                  />

                  {showItemSearch && debouncedItemSearch.length >= 2 && (
                    <div className="absolute z-[9999] mt-2 w-full overflow-hidden rounded-unit-lg border border-[var(--unit-border)]/50 bg-[var(--unit-surface)] shadow-unit-sm" style={{ maxHeight: '400px' }}>
                ...
                <div className="flex flex-wrap gap-3">
                  {servicesForUnit.map((s: { id: string; name: string; price?: unknown; unit: string; priceType?: string; minPrice?: number; maxPrice?: number; requiresApproval?: boolean }) => (
                    <button
                      ...
                      type="button"
                      onClick={() => addServiceToCart(s)}
                      className="relative flex flex-col items-center gap-2 rounded-unit border border-[var(--unit-border)]/50 bg-[var(--unit-surface)] px-4 py-3 text-sm font-medium transition-all hover:border-[var(--unit-accent)]/50 active:scale-[0.98] group"
                    >
                      <div className="flex items-center gap-2">
          ...
          <div className="space-y-4 lg:sticky lg:top-20 lg:self-start">
            <div className="rounded-unit-lg border border-[var(--unit-border)]/30 bg-[var(--unit-surface-elevated)]">
              {/* Ticket header */}
              <div className="bg-[var(--unit-accent)]/5 px-6 py-4 border-b border-[var(--unit-border)]/30">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-unit bg-[var(--unit-accent)]">
                  ...
                  {cart.length > 0 && (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => setCart([])}
                        className="rounded-unit p-2 text-[var(--unit-text)] transition-colors hover:bg-red-500/15 hover:text-red-500"
                ...
                {cart.length === 0 ? (
                  ...
                  <>
                    <ul className="space-y-3 mb-4">
                      {cart.map((i, idx) => (
                        <li key={idx} className="group rounded-[var(--unit-radius-sm)] border border-[var(--unit-border)] bg-[var(--unit-surface)] p-3 transition-all hover:border-[var(--unit-accent)]">
                        ...
                        <div className="space-y-2">
                          <div className="flex gap-2 items-center">
                            <input
                              ...
                              placeholder="Monto"
                              className="w-24 rounded-[var(--unit-radius-sm)] border border-[var(--unit-border)] bg-[var(--unit-surface)] px-2 py-1.5 text-xs text-[var(--unit-text-muted)] text-center focus:outline-none focus:ring-1 focus:ring-[var(--unit-accent)]"
                            />
                            <input
                              type="text"
                              ...
                              className="min-w-0 flex-1 rounded-[var(--unit-radius-sm)] border border-[var(--unit-border)] bg-[var(--unit-surface)] px-2 py-1.5 text-xs text-[var(--unit-text-muted)] focus:outline-none focus:ring-1 focus:ring-[var(--unit-accent)]"
                            />
                          </div>
              ...
              <div className="border-t border-dashed border-[var(--unit-border)] bg-[var(--unit-surface)] px-6 py-4 space-y-3">
                ...
                <button
                  ...
                  className="mt-3 w-full min-h-[48px] touch-manipulation rounded-unit bg-[var(--unit-accent)] text-white py-3.5 text-sm font-bold uppercase tracking-wider transition-all active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
        ...
        {/* Sale Closed Modal */}
        {lastClosedSale && (
          <div className="fixed inset-0 z-10 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4">
            <div className="w-full max-w-sm rounded-t-[calc(var(--unit-border-radius)*1.5)] sm:rounded-unit-lg border border-[var(--unit-border)]/30 bg-[var(--unit-surface-elevated)] p-6 sm:p-8">
              <div className="mx-auto mb-6 h-1 w-10 rounded-full bg-[var(--unit-text)]/20 sm:hidden" />

              <div className="mb-8 text-center">
                <div className="relative mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-unit-lg bg-[var(--unit-success)]/15">
                  <ShoppingBag className="relative h-8 w-8 text-[var(--unit-success)]" />
                  <div className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-[var(--unit-success)] flex items-center justify-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-3xl font-bold text-[var(--unit-success)] tabular-nums">S/ {lastClosedSale?.total?.toFixed(2) || '0.00'}</p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full rounded-unit bg-[var(--unit-accent)] py-3.5 text-sm font-bold text-white transition-all active:scale-[0.98]"
                >
                ...
                <button
                  ...
                  className="w-full rounded-unit border border-[var(--unit-border)]/50 bg-[var(--unit-surface)] py-3.5 text-sm font-semibold text-[var(--unit-text-muted)] transition-all hover:border-[var(--unit-accent)]/50 hover:text-[var(--unit-accent)]"
                >
                  Continuar
                </button>
              </div>
        ...
        {receiptHtmlToPrint && (
          <div className="fixed inset-0 z-60 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4">
            <div className="w-full max-w-md rounded-t-[calc(var(--unit-border-radius)*1.5)] sm:rounded-[var(--unit-border-radius)] border border-[var(--unit-border)] bg-[var(--unit-surface)] p-5 sm:p-6 shadow-[var(--unit-shadow-unit)]">
              ...
              <iframe
                ...
                title="Comprobante"
                className="w-full h-96 rounded-[var(--unit-radius-sm)] border border-[var(--unit-border)] bg-[var(--unit-surface-elevated)]"
              />
        ...
        {showEmployeeModal && selectedServiceForEmployee && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
            <div className="rounded-unit-lg border border-[var(--unit-border)]/30 bg-[var(--unit-surface-elevated)] p-6 max-w-md w-full">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-unit bg-[var(--unit-accent)]">
                ...
                <button
                  ...
                  className="rounded-unit p-2 text-[var(--unit-text-muted)] transition-all hover:bg-red-500/15 hover:text-red-500"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div className="rounded-unit border border-[var(--unit-border)]/50 bg-[var(--unit-surface)] p-4">
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {employees.length === 0 ? (
                      <div className="text-center py-8 rounded-unit border border-[var(--unit-border)]/50 bg-[var(--unit-surface)]">
                      ...
                      employees.map((employee: any) => (
                        <button
                          ...
                          className="w-full text-left p-4 rounded-unit border border-[var(--unit-border)]/50 bg-[var(--unit-surface)] transition-all hover:border-[var(--unit-accent)]/50 group"
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex h-10 w-10 items-center justify-center rounded-unit bg-[var(--unit-accent)]/10 group-hover:bg-[var(--unit-accent)]/20 transition-colors">
                            ...
                            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-[var(--unit-accent)]/10 group-hover:bg-[var(--unit-accent)]/20 transition-colors">
                              <ChevronRight className="h-3 w-3 text-[var(--unit-accent)]" />
                            </div>
                          </div>
                        </button>
                      ))
              ...
              <div className="mt-6">
                <button
                  ...
                  className="w-full rounded-unit border border-[var(--unit-border)]/50 bg-[var(--unit-surface)] px-4 py-3 text-sm font-semibold text-[var(--unit-text-muted)] transition-all hover:border-red-500/50 hover:bg-red-500/10 hover:text-red-500 active:scale-[0.98]"
                >
                  Cancelar
                </button>
              </div>
        ...
        {showCashRegisterWarning && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="bg-[var(--unit-surface-elevated)] rounded-unit-lg border border-[var(--unit-border)]/30 p-8 max-w-md w-full">
              <div className="flex items-center justify-center w-16 h-16 bg-red-100 rounded-full mx-auto mb-6">
                <Lock className="h-8 w-8 text-red-600" />
              </div>
              <h3 className="text-2xl font-bold text-center text-[var(--unit-text)] mb-4">
                Caja Cerrada
              </h3>
              <p className="text-[var(--unit-text-muted)] text-center mb-8">
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 px-6 py-3 border border-[var(--unit-border)] text-[var(--unit-text)] rounded-unit font-medium hover:bg-[var(--unit-surface)] transition-colors"
                >
                  Entendido
                </button>
                <Link
                  href="/cash-register"
                  className="flex-1 px-6 py-3 bg-[var(--unit-success)] text-white rounded-unit font-medium hover:opacity-90 transition-colors text-center"
                >
                  Abrir Caja
                </Link>
              </div>
