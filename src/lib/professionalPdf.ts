/**
 * Función unificada para generar PDFs profesionales con diseño corporativo.
 * Basado en el diseño premium de pdfReport.ts pero parametrizable para reutilización.
 */
...
// 🎨 CONSTANTES PREMIUM (reutilizadas de pdfReport.ts)
const MARGIN = 20;
 ...
 * Reutiliza la lógica premium de pdfReport.ts de forma parametrizable.
 */
export function generateProfessionalPdf(options: ProfessionalPdfOptions): void {
  ...
  const headerColor = options.customStyles?.headerColor || colors.primary;
  ...
  if (options.businessInfo?.name || options.includeLogo) {
    const businessName = options.businessInfo?.name || 'Barbería & Spa POS';
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(businessName, MARGIN + 12, y + 6);
    y += 10;

    // Información de contacto
    const contactInfo = [
      options.businessInfo?.address,
      options.businessInfo?.phone && `📞 ${options.businessInfo.phone}`,
      options.businessInfo?.email && `✉️ ${options.businessInfo.email}`
    ].filter(Boolean).join(' | ');
  ...
  const subtitleParts = [
    ...
    options.periodInfo && (
      ...
      `Período: ${format(options.periodInfo.from, "d 'de' MMMM 'de' yyyy", { locale: es })} al ${format(options.periodInfo.to, "d 'de' MMMM 'de' yyyy", { locale: es })}`
    ),
  ...
  if (subtitleParts.length > 0) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(255, 255, 255);
    const reportSubtitle = subtitleParts.join(' | ');
  ...
  // 🎨 ESTADÍSTICAS PREMIUM (si hay totales)
  if (options.totals) {
    ...
    doc.setFontSize(FONT_SIZE_HEADER);
    doc.setFont('helvetica', 'bold');
    doc.text(options.totals.label || 'TOTAL DEL REPORTE:', MARGIN + 5, y + 8);
    
    doc.setTextColor(colors.warning[0], colors.warning[1], colors.warning[2]);
    doc.setFontSize(FONT_SIZE_TITLE);
    doc.text(`${options.totals.currency || 'S/'} ${options.totals.amount.toFixed(2)}`, PAGE_WIDTH - MARGIN - 5, y + 8, { align: 'right' });
  ...
  let rowCount = 0;
  const alternateRowColor = options.customStyles?.alternateRowColor || colors.light;
  ...
  for (const row of options.rows) {
    ...
    // 🎯 COLORES POR COLUMNA (similares a pdfReport.ts)
    row.forEach((cell, i) => {
      const x = MARGIN + i * colWidth + 2;
      let cellColor = colors.dark;
      
      // Aplicar colores según tipo de columna (pueden personalizarse)
      switch (i) {
        case 0: // ID - Gris
          cellColor = colors.dark;
          break;
        case 1: // Concepto/Nombre - Oscuro
          cellColor = colors.dark;
          break;
        case 2: // Monto - Ámbar
      ...
      if (i === 2 && typeof cell === 'number') {
        cellText = `${options.totals?.currency || 'S/'} ${cell.toFixed(2)}`;
  ...
  const businessName = options.businessInfo?.name || 'Barbería & Spa POS';
  const footerText = `© ${new Date().getFullYear()} ${businessName} - Página ${doc.getNumberOfPages()} - Reporte generado profesionalmente`;
